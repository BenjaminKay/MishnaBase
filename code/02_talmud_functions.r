...
dropalpha <- function(col_vec){
  # Extract the numbers in a string vector by removing all the alpha characters (That will match alpha-chars in all Unicode alphabet languages. )
  # https://stackoverflow.com/questions/14543627/extracting-numbers-from-vectors-of-strings/38712300
  return((gsub("[[:alpha:]]", "", col_vec)))
}

droppunct <- function(col_vec){
  # Remove all punctuation
  # https://www.rexegg.com/regex-quickstart.html
  return((gsub("[[:punct:]]", "", col_vec)))
}

keepdigits <- function(col_vec){
  # Remove everything except digits (does not keep  decimals or negative signs)
  # https://stackoverflow.com/questions/52722846/how-can-i-remove-non-numeric-characters-from-strings-using-gsub-in-r/52729957
  # Could use this for a float type number gsub("[^0-9.-]", "", x)
  return((gsub("[^0-9]", "", col_vec)))
}
...
AssignScores <- function(df_input){
  ...
  # Landlord money issues (BHB) recall BHB = 0.0 is the stricter argument (as above with strict)
  df_input$Money_1 <- (df_input$Result_1 == "BHB 0.0") * 1
  df_input$Money_2 <- (df_input$Result_2 == "BHB 0.0") * 1
  df_input$Money_3 <- (df_input$Result_3 == "BHB 0.0") * 1    
  df_input$Money_4 <- (df_input$Result_4 == "BHB 0.0") * 1
  df_input$Money_5 <- (df_input$Result_5 == "BHB 0.0") * 1      
  df_input$Money_6 <- (df_input$Result_6 == "BHB 0.0") * 1   
...
# Old way, only works correctly on cases with 2 or fewer disputes
...
# New way, robust to N people in a position
UngroupNames2 <- function (df_input){
  # If an opinion is shared then we need to duplicate the row but put one name from the split in each row
  # This has to be done for each disputant column. 
  #This one does one
  for(i in 1:nrow(df_input)) {
      Disputant_1 = unlist(df_input[i,]$Disputant_1)
      if (grepl("/", Disputant_1, fixed=TRUE)) {
          NumberRabbisInPosition_1 = str_count(Disputant_1, "/") + 1
          names_1 = (unlist(strsplit((Disputant_1), "/")))
          df_input[i,]$Disputant_1 <-names_1[1]  

          for(j in 2:NumberRabbisInPosition_1) {
            df_input[nrow(df_input) + 1,]           = df_input[i,]
            df_input[nrow(df_input),]$Disputant_1   = names_1[j]  
            #print(paste(i, j, names_1[j]))
          }
  ...
  for(i in 1:nrow(df_input)) {
      ...
      Disputant_2 = unlist(df_input[i,]$Disputant_2)
      if (grepl("/", Disputant_2, fixed=TRUE)) {
          NumberRabbisInPosition_2 = str_count(Disputant_2, "/") + 1        
          names_2 = (unlist(strsplit((Disputant_2), "/")))
           # df_input[i,]$Disputant_2  <-names_2[1]   
          df_input[i,]$Disputant_2 <-names_2[1]  

          for(j in 2:NumberRabbisInPosition_2) {
            # df_input[nrow(df_input) + 1,] = df_input[i,]
            # df_input[i,]$Disputant_2  <- names_2[j]  
            df_input[nrow(df_input) + 1,]           = df_input[i,]
            df_input[nrow(df_input),]$Disputant_2   = names_2[j]              
          }
...
# df_scores4 %>% filter(SeqID == 1214) %>% select(Disputant_1, Disputant_2, SeqID, tabnum)
# test <- UngroupNames2(df_scores4 %>% filter(SeqID == 1214))
# test %>% select(Disputant_1, Disputant_2, SeqID, tabnum, DisputeID)


# df_scores4 %>% filter(SeqID == 778) %>% select(Disputant_1, Disputant_2, SeqID, tabnum)
# test <- UngroupNames2(df_scores4 %>% filter(SeqID == 778))
# test %>% select(Disputant_1, Disputant_2, SeqID, tabnum, DisputeID)

# df_scores4 %>% filter(SeqID %in% c(1214, 778)) %>% select(Disputant_1, Disputant_2, SeqID, tabnum)
# test <- UngroupNames2(df_scores4 %>% filter(SeqID %in% c(1214, 778)))
# test %>% select(Disputant_1, Disputant_2, SeqID, tabnum, DisputeID)


#Makes all disputes into two dispute setups
# Takes an N way argument (N positions, not N disputants, because each position may be shared
#  by multiple disputants) and makes it into a series of 2 way disputes.
# For example, if there are three positions A,  B, and C, this will make 
# the origional row into AB, and make two new rows AC and BC.
# However, if A is a two rabbi positions A1A2 versus B, it will stay as one row. 
# The spitting into multiple rows needed for that is handled by the function "UngroupNames2"
      ...
      for (ii in 1:(maxval-1)) { #need to loop through one time fewer than the number of disputes 
        for (ij in ii:maxval){
          if (ii!=ij){ # Make sure you don't compare a rabbi to themselves
              ...
              Result_Field1_Str    <- paste("Result",    ii, sep="_")
              ...
              Result_Field2_Str    <- paste("Result",    ij, sep="_")
              ...
              df_output[output_rows + il, ]$Result_1  <- df_input[ik, Result_Field1_Str]
              df_output[output_rows + il, ]$Result_2  <- df_input[ik, Result_Field2_Str]
...
CullNonWinners <- function(df_input){
  # This version culls all the blanks, which results in no winner
...
# Handle non 1 / 0 strict results
  ...
  # Notice that sapply with "[" can be used to extract either the first or second items in those lists 
  # so  this captures the numeric part based on the space
...
# Need to do the same thing for OWE and BHB
# Owe Handle non 1 / 0 strict results
  df_input$Owe_Result_1 <- ""
  df_input$Owe_Result_2 <- ""

  df_input$Owe_Result_1[df_input$OweQuestion==1] <- df_input %>% filter(OweQuestion==1) %>% 
     mutate(Owe_Result_1n = Result_1) %>% pull()   
  df_input$Owe_Result_2[df_input$OweQuestion==1] <- df_input %>% filter(OweQuestion==1) %>% 
    mutate(Owe_Result_2n = Result_2) %>% pull()   

  df_input$Owe_Result_Score_1 <- sapply(strsplit(as.character(df_input$Owe_Result_1),' '), "[", 2)
  df_input$Owe_Result_Score_2 <- sapply(strsplit(as.character(df_input$Owe_Result_2),' '), "[", 2)

# BHB  Handle non 1 / 0 strict results
  df_input$Money_Result_1 <- ""
  df_input$Money_Result_2 <- ""

  df_input$Money_Result_1[df_input$MoneyQuestion==1] <- df_input %>% filter(MoneyQuestion==1) %>% 
     mutate(Money_Result_1n = Result_1) %>% pull()   
  df_input$Money_Result_2[df_input$MoneyQuestion==1] <- df_input %>% filter(MoneyQuestion==1) %>% 
    mutate(Money_Result_2n = Result_2) %>% pull()   

  df_input$Money_Result_Score_1 <- sapply(strsplit(as.character(df_input$Money_Result_1),' '), "[", 2)
  df_input$Money_Result_Score_2 <- sapply(strsplit(as.character(df_input$Money_Result_2),' '), "[", 2)
...
###############
  ...
  # 2/27/22 -- overwrote store and not result. Not a big deal because only using alt version
  # df_input$Strict_Result_Score_1 = df_input$AltStrict_1 
  # df_input$Strict_Result_Score_2 = df_input$AltStrict_2
  df_input$Strict_Result_1 = df_input$AltStrict_1 
  df_input$Strict_Result_2 = df_input$AltStrict_2
  ...
  df_input2 = df_input2 %>% mutate(
    ...
    # ),
      AltStrict_1==1 & AltStrict_2==0 ~ Disputant_2,
      AltStrict_1==0 & AltStrict_2==1 ~ Disputant_1,
      AltStrict_1==0 & AltStrict_2==0 ~ Disputant_2 # Note however that we have to set Tie.Win = True
    ),    
    ...
    # ),   
      AltStrict_1==1 & AltStrict_2==0 ~ Disputant_1,
      AltStrict_1==0 & AltStrict_2==1 ~ Disputant_2,
      AltStrict_1==0 & AltStrict_2==0 ~ Disputant_1 # Note however that we have to set Tie.Win = True
    ),       
    ...
    # ),
      AltMoney_1==1 & AltMoney_2==0 ~ Disputant_2,
      AltMoney_1==0 & AltMoney_2==1 ~ Disputant_1,
      AltMoney_1==0 & AltMoney_2==0 ~ Disputant_2 # Note however that we have to set Tie.Win = True
    ),    
    ...
    # ),  
      AltMoney_1==1 & AltMoney_2==0 ~ Disputant_1,
      AltMoney_1==0 & AltMoney_2==1 ~ Disputant_2,
      AltMoney_1==0 & AltMoney_2==0 ~ Disputant_1 # Note however that we have to set Tie.Win = True
    ),      
    ...
    # ),
      AltOwe_1==1 & AltOwe_2==0 ~ Disputant_1,
      AltOwe_1==0 & AltOwe_2==1 ~ Disputant_2,
...
metaseqcheck <- function(df_input_win, df_input_chumra, df_input_money, df_input_owe){
  ...
  seqcheck(df_input_win$WinnerName,     df_input_win$LoserName,     df_input_win$PsuedoDate,    df_input_win$Tie.Win)
  seqcheck(df_input_chumra$ChumraName,  df_input_chumra$KulaName,   df_input_chumra$PsuedoDate, df_input_chumra$Tie.Strict)  
  seqcheck(df_input_money$BHBhighName,  df_input_money$BHBlowName,  df_input_money$PsuedoDate,  df_input_money$Tie.Money)     
  seqcheck(df_input_owe$ChayavhighName, df_input_owe$ChayavlowName, df_input_owe$PsuedoDate,    df_input_owe$Tie.Owe)
  return(list(
...
BrandesResultsTwoWay <- function (df_scores, DispSupossedToWin, DispSupossedToLose){
  ...
  df_bran = rbind(
    df_bran_a %>%
      transmute(SeqID=SeqID, Tanna_1=Disputant_1, Tanna_2=Disputant_2, Win=Score_1, Lose=Score_2), 
      df_bran_b %>%
      transmute(SeqID=SeqID, Tanna_1=Disputant_2, Tanna_2=Disputant_1, Win=Score_2, Lose=Score_1)
      )
...
DisputesInvolving <- function (df_scores, TannaStr){
  # df_tmp = DisputesInvolving(df_scores100, "Tanna Kama")
  ...
  return(df_scores %>% filter(Disputant_1 == TannaStr | Disputant_2 == TannaStr))
}

# This shows that the dominance matrices strip out the ties
#1087 wins
# dim(df_scores100 %>% filter((Disputant_1 == "Tanna Kama" & Score_1==1)| (Disputant_2 == "Tanna Kama" & Score_2==1)))
#277 non wins
# dim(df_scores100 %>% filter((Disputant_1 == "Tanna Kama" & Score_1==0)| (Disputant_2 == "Tanna Kama" & Score_2==0)))
#76 ties which impliese that 201 losses
# dim(df_scores100 %>% filter((Disputant_1 == "Tanna Kama" & Score_1==0 & Score_2==0)| (Disputant_2 == "Tanna Kama" & Score_1==0 & Score_2==0)))
