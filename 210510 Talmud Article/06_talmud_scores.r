...
#########################################################################################
#################################### Handle dusputants 3-5. This approach creates two new rows to compare them against each. 
#########################################################################################
...
summary(df_scores2 %>% filter(NumDisp==2, Disputant_1 != "Tana Kama", Disputant_2 != "Tana Kama") %>% select(Strict_1))
summary(df_scores2 %>% filter(NumDisp==2, Disputant_1 != "Tana Kama", Disputant_2 != "Tana Kama") %>% select(Strict_2))
...
df_scores5 <- UngroupNames(df_scores4)

df_scores6a = left_join(df_scores5,  df_hebrew_rabbi_name_1 %>% select(English_Name, Hebrew_Name_1), by=c("Disputant_1"="English_Name"))
